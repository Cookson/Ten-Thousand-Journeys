...
// Handles all the key presses in game. 
	...
	var key = e.keyCode;
	var keyMode = Keys.mode;
//	console.log(keyMode, ctrl, alt, shift, key);
	var registeredAction;
	if (
		(keyMode == Keys.MODE_ON_GLOBAL_MAP || 
		keyMode == Keys.MODE_IN_LOCATION) &&
		Keys.keyActions[Keys.MODE_ALWAYS][ctrl][alt][shift][key]
	) {
	// Apply "default" action
//		console.log(Keys.keyActions[Keys.MODE_ALWAYS][ctrl][alt][shift][key])
		registeredAction = Keys.keyActions[Keys.MODE_ALWAYS][ctrl][alt][shift][key];
	} else {
...
Keys.setMode = function _(mode) {
	if (mode == Keys.MODE_DEFAULT) {
		Keys.mode = onGlobalMap ? Keys.MODE_ON_GLOBAL_MAP : Keys.MODE_IN_LOCATION;
	} else {
		Keys.mode = mode;
	}
};
