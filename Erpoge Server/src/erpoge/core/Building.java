...
public abstract class Building extends Rectangle {
	...
	public Coordinate placeDoor(RectangleArea r, Side side, Side sideOfSide,
			int depth, int object) {
		/**
		 * Places door in the particular cell on particular side of room
		 */
		Coordinate c = r.getCellFromSide(side, sideOfSide, depth).moveToSide(
				side, 1);
		settlement.setObject(c.x, c.y, object);
		return c;
	}
	...
	public HashMap<Integer, Integer> findDoorAppropriateCells(Rectangle r,
			Side side) {
		HashMap<Integer, Integer> cells = new HashMap<Integer, Integer>();
		Set<Integer> keys;
		if (side == Side.N) {
	...
	public RectangleSystem buildBasis(int wallType) {
		...
		for (int k : keys) {
			...
			for (int vertex : edge) {
				...
				Coordinate c = connectRoomsWithDoor(r1, r2, objDoorBlue);
	...
	public RectangleSystem getRectangleSystem(int minRoomSize) {
		rectangleSystem = settlement.getGraph(x, y, width, height, minRoomSize,
				1);
	...
	public ArrayList<Coordinate> getCellsNearWalls(Rectangle r) {
		...
		if (!settlement.isDoor(r.x, r.y - 1)
				&& !settlement.isDoor(r.x - 1, r.y)) {
	...
	public void clearBasisInside() {
		for (Rectangle r : rectangleSystem.content.values()) {
			settlement.square(r.x, r.y, r.width, r.height,
					TerrainBasics.ELEMENT_OBJECT, StaticData.VOID, true);
		}
	}
	...
	public void removeWall(Rectangle r, Side side) {
		...
		settlement.line(startX, startY, endX, endY,
				TerrainBasics.ELEMENT_OBJECT, StaticData.VOID);
	}
