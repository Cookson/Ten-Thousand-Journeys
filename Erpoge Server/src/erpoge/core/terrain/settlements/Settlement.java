...
public class Settlement extends Location {
	...
	public void markQuarter(EnhancedRectangle r, int width/* =8 */, int border/* =2 */) {
		...
		quarters.add(new RandomRectangleSystem(r.x + 1, r.y + 1, r.width - 2, r.height - 2, width, border));
	}
	...
	public class RoadSystem {
		...
		public void drawRoads() {
			...
			for (Road road : roads) {
				boldLine(road.start.x, road.start.y, road.end.x, road.end.y, ELEMENT_FLOOR, floorGround, 5);
			}
		}
	...
	public class QuarterSystem {
		...
		private void findQuarter(Coordinate point, int dx, int dy) {
			/**
			 * Find empty area from the particular side of point. Side is
