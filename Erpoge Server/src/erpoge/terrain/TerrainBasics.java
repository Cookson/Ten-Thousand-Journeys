...
//		Main.console(x+" "+y+" "+cells[x][y]);
	...
	public void computePassability(int x, int y) {
		...
		if (c.floor() == GameObjects.FLOOR_WATER) {
			cells[x][y].setPassability(1);
		} else if (c.object() != GameObjects.OBJ_VOID) {
			cells[x][y].setPassability(GameObjects.getPassability(c.object()));
		} else if (c.character() != null) {
			cells[x][y].setPassability(PASSABILITY_SEE);
		} else {
			cells[x][y].setPassability(PASSABILITY_FREE);
		}
	}
